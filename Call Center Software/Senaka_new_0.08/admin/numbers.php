...
/**
 *  This file allows a logged in admin to view the list of telephone numbers
 * 
 */
...
if ($_SESSION['user_type'] != "admin" ) {
	// store error message in the session so that error can be displayed in next page
...
if(!isset($db_error) && !isset($error) ) {
	...
	if (!isset($cursor)){
		...
		$fmt = "<input type='hidden' name='phone' value='%s'/><tr> <td>
		<a href=\"#\" onclick=\"document.getElementById('%s').submit(); return false;\">%s</a>
		</td><td>%s</td></tr>\n";
		$count = 0;
		while ($cursor->hasNext()) {
			$res = $cursor->getNext();
			$formName = $count."??form";
			$output[] .= sprintf($fmt, $res['phone_number'], $formName,
					$res['phone_number'], $res['friendly_name']);
			$count++;
		}
	}
}
...
<div class="form-item">
	...
	<?php $count = 0; foreach($output as $val){ $formName = $count."??form";?>
	<form name="inputform" id="<?php echo $formName; ?>" method="post" action="numberdetail.php">
