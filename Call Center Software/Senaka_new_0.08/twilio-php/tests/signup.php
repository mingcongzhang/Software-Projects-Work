...
if (!isset($error) && isset($_POST['formsubmitted'])) {
	...
	if (empty($firstname) || !validate_name($firstname)) {
		...
		if ($result['status'] == "signup"){
			...
			$collection_company->insert(array('_id' => $company_id, 'companyname' => $companyname, 'account_status' => "active", 'price_plan' => "free1"));				
			...
			try{
				$collection_user->insert(array('password'=>$password,'email'=>$email,'phone'=>$phone,'firstname'=>$firstname,'lastname'=>$lastname,'code'=>$code,'type'=>"admin",'status'=>"signup",'company'=>$company_id));
			}	
...
<form action="<?php echo basename(__FILE__);?>" method="post" class="registration_form">
  <fieldset>
    ...
    <div class="elements">
      ...
      <input type="text" id="name" name="lastname"  value = "<?php echo $lastname ?>" size="50" /> (requried)
    </div>
    ...
    <div class="elements">
      ...
      <input type="text" id="name" name="phone" placeholder="10 or 11 digits number"  value = "<?php echo $phone ?>" size="50" /> (optional)
    </div>

    <div class="elements">
      <label for="cname">Company Name:</label>
      <input type="text" id="name" name="companyname"  value = "<?php echo $companyname ?>" size="50" /> (optional)
    </div>
