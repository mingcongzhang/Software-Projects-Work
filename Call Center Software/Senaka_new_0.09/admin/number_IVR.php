...
elseif(isset($_SESSION['edit_IVR'])&& $_POST['save_IVR_msg']){
	...
	$routeToChoiceArray = array();
	$availGreetingArray = array();
	$voicemailGreetingArray = array();
	$outsideGreetingArray = array();
	$welcomeGreetingArray = array();
	...
	foreach($_POST as $key => $value){		
		if(substr($key,0,9) === "tableName"){	
			...
			}elseif(strrpos($key,"_RouteTo")){
				$name = substr($key,0,strpos($key,"_RouteTo"));
				$routeToChoiceArray[$name] = $value;
			}elseif(strrpos($key,"availGreeting")){
				$name = substr($key,0,strrpos($key,"availGreeting"));
				$availGreetingArray[$name] = $value;
			}elseif(strrpos($key,"voiceGreeting")){
				$name = substr($key,0,strrpos($key,"voiceGreeting"));
				$voicemailGreetingArray[$name] = $value;
			}elseif(strrpos($key,"outsideGreeting")){
				$name = substr($key,0,strrpos($key,"outsideGreeting"));
				$outsideGreetingArray[$name] = $value;
			}elseif(strrpos($key,"welcomeGreeting")){
				$name = substr($key,0,strrpos($key,"welcomeGreeting"));
				$welcomeGreetingArray[$name] = $value;
	...
	try{
		$collection_phonenum->update(
			...
			'ivr_routeToOption_array'=>$routeToChoiceArray,
			'ivr_availGreeting_array'=>$availGreetingArray,
			'ivr_voicemailGreeting_array'=>$voicemailGreetingArray,
			'ivr_outsideGreeting_array'=>$outsideGreetingArray,
			'ivr_welcomeGreeting_array'=>$welcomeGreetingArray))),
			array('upsert'=>true)
		);
...
$result2 = $collection_group->findOne(array('_id'=>$company_id));
?>
...
var routeToOptionArray = <?php echo json_encode($result['ivr_routeToOption_array']); ?>;
var availAgentsGreetingArray = <?php echo json_encode($result['ivr_availGreeting_array']); ?>;
var voicemailGreetingArray = <?php echo json_encode($result['ivr_voicemailGreeting_array']); ?>;
var outsideBusinessGreetingArray = <?php echo json_encode($result['ivr_outsideGreeting_array']); ?>;
var welcomeGreetingArray = <?php echo json_encode($result['ivr_welcomeGreeting_array']); ?>;
var defaultWelcomeGreeting = <?php echo json_encode($result['welcome_greeting']); ?>;
var defaultAvailAgentsGreeting = <?php echo json_encode($result['available_agents_greeting']); ?>;
var defaultVoicemailGreeting = <?php echo json_encode($result['voice_message']); ?>;
var defaultOutsideBusinessGreeting = <?php echo json_encode($result['outside_hours_message']); ?>;
var defaultGroupArray = <?php echo json_encode($result2['label']); ?>;
</script>
