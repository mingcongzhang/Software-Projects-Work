...
//alert();
	...
	// }else{
		...
		cell3.innerHTML = '<select id = '+number_name+'><option selected="selected" value="">- Select an option -</option><option value="1">1</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5</option><option value="6">6</option><option value="7">7</option><option value="8">8</option><option value="9">9</option><option value="0">0</option></select>';	
...
function derived_choice(name,row,table){
		if(document.getElementById(name).selectedIndex == "0") {
			...
			var name_route = name+"_RouteTo";
			//alert(name_route);
			var tempHtml = '<select id ='+name_route+' name='+name_route+'><option value="">- Select an option -</option>';
			//var tempCount = 1;
			for(var group in defaultGroupArray){
				tempHtml += '<option value = '+defaultGroupArray[group]+'>'+defaultGroupArray[group]+'</option>';
				//tempCount++;
				//alert(defaultGroupArray[group]);
			}
			tempHtml += '</select>';
			cell6.innerHTML = tempHtml;
			if(typeof routeToOptionArray[name] != 'undefined'){
				$('#'+name_route).val(routeToOptionArray[name]);
			}else{
				$('#'+name_route).val("");
			}
			...
			var greetingName1 = name + "availGreeting";
			var htmlVar1 = '<select id = '+greetingName1+' name = '+greetingName1+'><option selected="selected" value="">- Select an option -</option>';
			if(defaultAvailAgentsGreeting != null){
				htmlVar1 += '<option value='+defaultAvailAgentsGreeting["name"]+'>'+defaultAvailAgentsGreeting["name"]+'</option>';
			}
			htmlVar1 += '</select>';
			added_cell3.innerHTML = htmlVar1;
			if(availAgentsGreetingArray != null){
				if(typeof availAgentsGreetingArray[name] != 'undefined'){
					$('#'+greetingName1).val(availAgentsGreetingArray[name]);
				}
			}
			//**********Voicemail Greeting
			...
			var greetingName2 = name + "voiceGreeting";
			var htmlVar2 = '<select id = '+greetingName2+' name = '+greetingName2+'><option selected="selected" value="">- Select an option -</option>';
			if(defaultVoicemailGreeting != null){
				htmlVar2 += '<option value='+defaultVoicemailGreeting["name"]+'>'+defaultVoicemailGreeting["name"]+'</option>';
			}
			htmlVar2 += '</select>';
			added_cell3.innerHTML = htmlVar2;
			if(voicemailGreetingArray != null){
				if(typeof voicemailGreetingArray[name] != 'undefined'){
					$('#'+greetingName2).val(voicemailGreetingArray[name]);
				}
			}
			...
			var greetingName3 = name + "outsideGreeting";
			var htmlVar3 = '<select id = '+greetingName3+' name = '+greetingName3+'><option selected="selected" value="">- Select an option -</option>';
			if(defaultOutsideBusinessGreeting != null){
				htmlVar3 += '<option value='+defaultOutsideBusinessGreeting["name"]+'>'+defaultOutsideBusinessGreeting["name"]+'</option>';
			}
			htmlVar3 += '</select>';
			added_cell3.innerHTML = htmlVar3;
			if(outsideBusinessGreetingArray != null){
				if(typeof outsideBusinessGreetingArray[name] != 'undefined'){
					$('#'+greetingName3).val(outsideBusinessGreetingArray[name]);
				}
			}
			...
			added_cell1.innerHTML = '<label name ="special" style="white-space: nowrap;">&nbsp;&nbsp;&nbsp;</label> ';
			var added_cell2 = added_row1.insertCell(1);	
			var greetingName = table_name + "welcomeGreeting";
			var htmlVar = '<div style="border: 2px solid #a1a1a1;padding: 6px;background: #dddddd;width: 290px;height: 25px;border-radius: 25px;"><div style="text-align: center;"><label style="white-space: nowrap;">Sub-IVR Message:</label><select  id = '+greetingName+' name = '+greetingName+'><option selected="selected" value="">- Select an option -</option>';
			if(defaultWelcomeGreeting != null){
				htmlVar += '<option value='+defaultWelcomeGreeting["name"]+'>'+defaultWelcomeGreeting["name"]+'</option>';
			}
			htmlVar += '</select></div></div>';
			added_cell2.innerHTML = htmlVar;
			if(welcomeGreetingArray != null){
				if(typeof welcomeGreetingArray[table_name] != 'undefined'){
					$('#'+greetingName).val(welcomeGreetingArray[table_name]);
				}
			}
			var added_cell3 = added_row1.insertCell(2);
