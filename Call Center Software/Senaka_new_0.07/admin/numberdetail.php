...
/**
 *  This file enables an admin to get details of, and customize a telephone number 
 * 
 */
...
if( !isset($db_error, $error, $_POST['number'] ) && $_POST['number'] == "showdetail" ) {
	...
	if (empty($phone) ) {
		...
		// Update telephone number details from the database, filtered by company ID
		try {
...
// This is when the user updates and submits information on a particular telephone number
	...
	// Get the telephone number (e.g. "+18475765000"), in order to update the phone info.
	$phone = isset($_POST['phone']) ? $_POST['phone']: null;
	...
	header("Location: http://".$_SERVER['SERVER_NAME'].DIRECTORY_SEPARATOR."~".$token[sizeof($token)-3].DIRECTORY_SEPARATOR."admin/numbers.php");
}
