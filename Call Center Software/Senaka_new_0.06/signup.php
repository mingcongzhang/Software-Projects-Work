...
if (!isset($error) && isset($_POST['formsubmitted'])) {
	...
	if (empty($firstname) || !validate_name($firstname)) {
		...
		if ($result['status'] == "signup"){
			...
			try{
				...
				'sid' => $account->sid, 'twilio_friendly' => $account->friendly_name,
				'twilio_status' => $account->status, 'twilio_created'=>$account->date_created,
				'twilio_updated'=>$account->date_updated,'token' =>$account->auth_token,
				'twilio_type' => $account->type,'last_update'=>time()
				));
				$collection_group->insert(array('_id'=>$company_id,'label'=>array("Admin","Agent")));
			...
			try{
				...
				'type'=>"admin",'status'=>"signup",'company'=>$company_id, 'online_status'=>"0", 'last_update'=>time()
));
			}	
			catch(MongoException $e){
				$error = "Failed to insert data in Database. Please try again!";
	...
	if (!isset($error)){
		...
		$sitepath = ($ACTIVATION_DIR.DIRECTORY_SEPARATOR.'~'.$token[sizeof($token)-2].DIRECTORY_SEPARATOR.'activate.php').'?email='.urlencode($email).'&key='.urlencode($code);
...
<form action="<?php echo basename(__FILE__);?>" method="post" class="registration_form">
  <fieldset>
    ...
    <div class="elements">
      ...
      <input type="text" id="name" name="firstname" value = "<?php if(isset($firstname)){ echo $firstname;} ?>" size="50" /> (required)
    </div>

    <div class="elements">
      <label for="lname">Last Name:</label>
      <input type="text" id="name" name="lastname"  value = "<?php if(isset($lastname)){ echo $lastname;} ?>" size="50" /> (required)
    </div>
    
    <div class="elements">
      <label for="e-mail">E-mail:</label>
      <input type="text" id="e-mail" name="e-mail" placeholder="Email Address"  value = "<?php if(isset($email)){ echo $email;} ?>" size="50" /> (required)
    </div>
    ...
    <div class="elements">
      ...
      <input type="text" id="name" name="phone" placeholder="10 or 11 digits number"  value = "<?php if(isset($phone)){ echo $phone;} ?>" size="50" /> 
    </div>

    <div class="elements">
      <label for="cname">Company Name:</label>
      <input type="text" id="name" name="companyname"  value = "<?php if(isset($companyname)){ echo $companyname;} ?>" size="50" /> 
    </div>
