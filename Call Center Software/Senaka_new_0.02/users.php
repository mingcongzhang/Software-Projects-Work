...
if(!isset($db_error) && $_POST['contact'] == "add" ) {
	...
	if(!isset($error)){
		...
		try{
			$collection_user->insert(array('last_update'=>time(),'password'=>$code,'email'=>$email,'phone'=>$phone, 'firstname'=>$firstname,'lastname'=>$lastname,'code'=>$code,'type'=>$type, 'status'=>"signup", 'company'=>$company_id, 'queue'=>$q_selected));
		}	
