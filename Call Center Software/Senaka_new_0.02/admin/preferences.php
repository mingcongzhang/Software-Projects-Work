...
require_once("/home/mingcong/public_html/includes/select_list.php"); 
...
if(!isset($error)&&isset($company_id)&&isset($_POST['submit'])){
	...
	$_SESSION['queue_size'] = $_POST['select_queuesize'];
	$groups = $_POST['groupInput'];
	$cursor = $collection_group->findOne(array('_id'=>$company_id));
	if(!isset($cursor)){
		try{
		$collection_group->insert(array('_id'=>$company_id,'label'=>$groups));
		}
		...
		$_SESSION['groups_new'] = array_merge($groups, $groups_origin);
		try{
		$collection_group->update(array('_id'=>$company_id),array('$set'=>(array('label'=>$_SESSION['groups_new']))),array('upsert'=>false));
		}
		catch(MongoException $e) {
	...
	try{
		$collection_company->update(array('_id'=>$company_id),array('$set'=>(array('last_update'=>time(),'timezone'=>$timezone,'timeout_inactivity'=>$timeout_inactivity,'business_hours'=>$business_hours,'start_time'=>$start_time,'end_time'=>$end_time,'inboundcall_recording'=>$inboundcall_recording,'outboundcall_recording'=>$outboundcall_recording,'number_agents_ring'=>$number_agents_ring,'queue_size'=>$queue_size))),array('upsert'=>false));
	}
	catch(MongoException $e) {
		die("Failed to update data ".$e->getMessage());
	}
	...
	if($result_company == NULL){
		...
		$companyname_get = $result_company['companyname'];
	}
...
<fieldset>
      <div class="inner-welcome-box">
       <h1>Preferences</h1>
		<?php if(!isset($error)){?>
		<h2><?php echo $_SESSION['companyname_get']; ?></h2>
		<?php } ?>
      </div>
<!--===============error msg============================-->
<?php
...
<form action="<?php echo basename(__FILE__);?>" method="post">
     <div id="dynamicInput">
          Group 1<br><input type="text" name="groupInput[]">
     </div>
...
if(!isset($_SESSION['clicked'])){
	...
	foreach($time_array as $key=>$val){
		echo ($key == $default) ? "<option selected=\"selected\" value=\"$key\">$val</option>":"<option value=\"$key\">$val</option>"; } unset($starttime);?>
	      </select>
      To:
...
if(!isset($_SESSION['clicked'])){
	...
	$queuesize = $_SESSION['queue_size_get'];
}
